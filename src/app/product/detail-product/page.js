...
export default function DetailProduct() {
    ...
    if (token) {
      if (fetchData) {
        const fetchDetailProductData = async () => {
          try {
            const res = await axios.get(PRODUCT_URL("getbyid", idProduct), {
              headers: {
                Authorization: `Bearer ${token}`,
              },
            });
  ...
  return (
    <>
      {/* DESKTOP MODE
        section : xl:w-[70%] xl:mx-auto */}
      {/* MOBILE MODE */}
      <section className="h-screen font-poppins xl:w-[70%] xl:mx-auto">
        <div className="px-4">
          <div
            onClick={() => router.back()}
            className="fixed inset-x-0 top-0  flex cursor-pointer items-center gap-6 bg-ngaos-4  px-[16px] py-[10px] text-white ">
            <FiArrowLeft className="h-[30px] w-[30px]" /> <h1>Detail Produk</h1>
          </div>
          ...
          <div className="mt-[80px] flex flex-col gap-2">
            ...
            <div className="flex flex-col gap-1">
              ...
              <textarea
                ...
                placeholder="Penjelasan Produk"></textarea>
            </div>
            {/* penjelasan produk */}
            ...
            <div className="flex flex-col gap-1">
              ...
              <Image alt="" src={showImage} width={200} height={200} />
