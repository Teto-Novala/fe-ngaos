...
export default function Product() {
  ...
  const handleDelete = async (idProduct) => {
    if (token) {
      try {
        const res = await axios.delete(PRODUCT_URL("delete", idProduct), {
          headers: {
  ...
  return (
    <>
      {/* DESKTOP MODE
        session : xl:flex
        content : xl:order-2 xl:w-full */}
      {/* MOBILE MODE */}
      <section className="h-screen font-poppins xl:flex">
        {/* content */}
        <div className="px-4 xl:order-2 xl:w-full">
          ...
          <div className="grid grid-cols-12 mt-5 gap-5">
            ...
            <Button
              ...
              className=" rounded-rad-4 py-2 col-span-4 bg-ngaos-4 text-white px-3 xl:w-1/2 ">
              Tambah
            </Button>
          </div>
