...
export default function BottomNavbar() {
  ...
  return (
    // DESKTOP MODE
    // xl:static xl:order-1 xl:flex-col xl:h-screen
    <div className="fixed inset-x-0 bottom-0 flex h-[80px] justify-around bg-white p-2  text-center text-3xl shadow-low xl:static xl:order-1 xl:flex-col xl:h-screen ">
      <div
      ...
      <div
        ...
        className={`${
          pathname === "/transaction" ? "text-ngaos-4" : "text-ngaos-1"
        } flex cursor-pointer flex-col items-center justify-center gap-1`}>
        <BsCart2 />
      ...
      {role === "admin" && (
        <div
          onClick={() =>
            pathname === "/product" ? router.refresh() : router.push("/product")
          }
          className={`${
      ...
      <div
        onClick={() =>
          pathname === "/akun" ? router.refresh() : router.push("/akun")
        }
        className={`${
